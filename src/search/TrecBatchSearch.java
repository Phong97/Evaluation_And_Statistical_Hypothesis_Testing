...
public class TrecBatchSearch {
	...
	public static void main(String[] args) throws Exception {
		String usage =
				"Usage:\tjava TrecBatchSearch [-index indexDir] [-simfn similarity] [-field f] [-queries file] [-outDir outDir]";
		...
		File fileOut = new File(outDir + "/" + simstring);
		...
		while (true) {
			...
			doBatchSearch(in, searcher, pair[0], query, simstring, out);
		}
	 ...
	 */
	public static void doBatchSearch(BufferedReader in, IndexSearcher searcher, 
			String qid, Query query, String algorithm, BufferedWriter out)	 
			throws IOException {
		...
		for (int i = start; i < end; i++) {
			...
			out.write(qid + " Q0 " + docno + " " + (i + 1) + " " + hits[i].score + " " + algorithm + "\n");
		}
	}
}
